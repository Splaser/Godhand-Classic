...
function GDSpellIsAllClassInf(SpellIds,NoSpellTarget,BuffNames)
...
function GDSpellIsAllClassIgnoreInf(name,inf)
...
function GDTestSpellBook(SpellIds)
	...
	while true do
		...
		if spellName then
			...
			if not IsPassiveSpell(i,BOOKTYPE_SPELL) and not SpellIds[s] then
				if GDSpellIsAllClassIgnoreInf(spellName) then
...
function GDSpellIsMoveAll(Id,unit,name) --处理移动中能施放的技能
