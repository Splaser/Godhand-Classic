...
if not metaTable1 then
	...
	function HealCheck(tar)
		if ((UnitCanCooperate("player",tar)
		  ...
		  or SpecialHealUnitList[tonumber(select(2,getGUID(tar)))] ~= nil	or (UnitIsOtherPlayersPet(tar) or UnitGUID(tar) == UnitGUID("pet")))
...
--			print("calculating HP")
			local incomingheals;
			if UnitGetIncomingHeals(o.unit,"player") ~= nil then incomingheals = UnitGetIncomingHeals(o.unit,"player"); else incomingheals = 0; end
			local nAbsorbs;
	...
	function SetupTables() -- Creating the cache (we use this to check if some1 is already in the table)
		...
		function nNova:Update(MO)
			...
			if 1==1 then SpecialTargets = {}; end
